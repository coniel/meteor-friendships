User.methods({
    ...
    friends:function (limit, skip) {
        ...
        return Friend.collection.find({userId:this._id}, options).fetch();
    },
     ...
     */
    unfriend:function () {
        var friend = Friend.collection.findOne({userId:Meteor.userId(), friendId:this._id});

        //if we have a friend record, remove it. Friend.collection.after.remove will
    ...
    isFriendsWith: function (user) {
        ...
        return !!Friend.collection.findOne({userId: this._id, friendId:userId});
    }
