...
User.methods({
     ...
     */
    friendRequests: function (limit, skip) {
        var options = {limit: limit, skip: skip};
        return FriendRequest.collection.find({
            requestedUserId: this._id,
            denied: {$exists: false},
    ...
    hasRequestFrom: function (user) {
        var request = FriendRequest.collection.findOne({
            requestedUserId: this._id,
            userId: user._id
        }, {fields: {_id: true, denied: true}});
    ...
    requestFriendship: function () {
        ...
        new FriendRequest({requestedUserId: this._id, userId: Meteor.userId()}).save();
    },
    ...
    cancelFriendshipRequest: function () {
        var request = FriendRequest.collection.findOne({userId: Meteor.userId(), requestedUserId: this._id});
    ...
    acceptFriendshipRequest: function () {
        var request = FriendRequest.collection.findOne({userId: this._id, requestedUserId: Meteor.userId()});
